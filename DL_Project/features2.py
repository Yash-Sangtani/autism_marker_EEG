...
def recurrence_matrix(embedded_ts, radius):
    """
    Create a sparse recurrence matrix to reduce memory usage.
    ...
    sparse_matrix : scipy.sparse.csr_matrix
        Sparse representation of the recurrence matrix.
    """
    from scipy.spatial.distance import pdist, squareform
    from scipy.sparse import csr_matrix

    # Calculate pairwise distances
    distances = squareform(pdist(embedded_ts, metric='euclidean'))

    # Create a binary recurrence matrix and convert to sparse format
    recurrence_binary = (distances <= radius).astype(int)
    sparse_matrix = csr_matrix(recurrence_binary)
...
def extract_rqa_features(data, embedding_dimension=3, time_delay=1, radius=0.1, minimum_line_length=2):
    ...
    def batch_recurrence_matrix(embedded_ts, radius, batch_size=1000):
        """Create a sparse recurrence matrix using batch processing."""
        n_points = embedded_ts.shape[0]
        sparse_matrix = csr_matrix((n_points, n_points), dtype=int)

        for start_idx in range(0, n_points, batch_size):
            end_idx = min(start_idx + batch_size, n_points)
            batch = embedded_ts[start_idx:end_idx]

            distances = cdist(batch, embedded_ts, metric='euclidean')
            recurrence = (distances <= radius).astype(int)
            sparse_matrix[start_idx:end_idx, :] = csr_matrix(recurrence)

        return sparse_matrix

    # Compute sparse recurrence matrix in batches
    recurrence_matrix_data = batch_recurrence_matrix(embedded_ts, radius)

    # Recurrence Rate (RR)
    rr = recurrence_matrix_data.nnz / (recurrence_matrix_data.shape[0] ** 2)
    ...
    def diagonal_lines(matrix, min_length):
        ...
        for k in range(-rows + 1, cols):
            diag = matrix.diagonal(k).toarray().flatten()  # Extract diagonals from the sparse matrix
    ...
    det = sum(diag_lines) / recurrence_matrix_data.nnz if recurrence_matrix_data.nnz > 0 else 0
    ...
    def vertical_lines(matrix, min_length):
        ...
        for col in range(matrix.shape[1]):
            column = matrix[:, col].toarray().flatten()  # Extract columns from the sparse matrix
    ...
    lam = sum(vert_lines) / recurrence_matrix_data.nnz if recurrence_matrix_data.nnz > 0 else 0
...
def main():
    ...
    data_path = 'C:/Users/Dhruv/PycharmProjects/DeepLearning/autism_marker_EEG/Aging/'
    output_path = 'C:/Users/Dhruv/PycharmProjects/DeepLearning/autism_marker_EEG/features2/'
    combined_csv_path = ('C:/Users/Dhruv/PycharmProjects/DeepLearning/autism_marker_EEG'
                         '/features_combined2.csv')
